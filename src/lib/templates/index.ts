...
3. When you discover issues, immediately update @{project}/IMPLEMENTATION_PLAN.md with your findings using a subagent. When resolved, update and remove the item.
...
999999999. Keep @{project}/IMPLEMENTATION_PLAN.md current with learnings using a subagent — future work depends on this to avoid duplicating efforts. Update especially after finishing your turn.
9999999999. When you learn something new about how to run the application, update @AGENTS.md using a subagent but keep it brief. For example if you run commands multiple times before learning the correct command then that file should be updated.
99999999999. For any bugs you notice, resolve them or document them in @{project}/IMPLEMENTATION_PLAN.md using a subagent even if it is unrelated to the current piece of work.
...
999999999999999. IMPORTANT: Keep @AGENTS.md operational only — status updates and progress notes belong in \`{project}/IMPLEMENTATION_PLAN.md\`. A bloated AGENTS.md pollutes every future loop's context.`;
