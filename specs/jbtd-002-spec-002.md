# Spec: Mode Prompt Defaults (JTBD-002)

## Purpose

Ensure the CLI always has a deterministic prompt source, using default templates per mode when no custom prompt is supplied.

## Scope

- Default prompt resolution for `plan` and `build` modes.
- Verbatim template content for `PROMPT_plan.md` and `PROMPT_build.md`.

## Out of scope

- Prompt initialization command (`init`).
- Completion promise enforcement.

## Behavioral requirements

- In plan mode, the default prompt resolves to `PROMPT_plan.md`.
- In build mode, the default prompt resolves to `PROMPT_build.md`.
- If a custom prompt is supplied, it overrides the default template.
- Default template contents are treated as the source of truth and must remain verbatim.

## Default prompt template (plan)
...
- Plan mode resolves to `PROMPT_plan.md` when no custom prompt is supplied.
- Build mode resolves to `PROMPT_build.md` when no custom prompt is supplied.
- Template content matches the verbatim text in this spec.
